/**
 * seedNavigation.js
 * Run once to populate MongoDB with all navigation module data.
 * Usage: node ser/scripts/seedNavigation.js
 *
...
async function seed() {
    ...
    console.log('Connected. Seeding navigation data...\n');

    for (const navModule of navigationData) {
        const result = await Navigation.findOneAndUpdate(
            { moduleKey: navModule.moduleKey },
            { $set: navModule },
            { upsert: true, new: true, setDefaultsOnInsert: true }
        );
        console.log(`  ✔  ${navModule.moduleKey} (${navModule.title}) — saved`);
    }

    console.log(`\nDone! ${navigationData.length} modules seeded.`);
