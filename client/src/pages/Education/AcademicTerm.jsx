...
export default function AcademicTerm() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await API.get('/api/resource/Academic Term?fields=["name","academic_year","term_start_date","term_end_date"]&limit_page_length=None&order_by=term_start_date desc');
            if (res.data.data) setData(res.data.data);
        } catch (err) {
            console.error('Fetch failed:', err);
            notification.error({ message: 'Failed to load Academic Terms' });
    ...
    const fetchMasters = async () => {
        try {
            const res = await API.get('/api/resource/Academic Year?fields=["name"]&limit_page_length=None&order_by=year_start_date desc');
            setAcademicYears((res.data.data || []).map(y => y.name));
        } catch (err) {
    ...
    const fetchSingle = async (name) => {
        try {
            const res = await API.get(`/api/resource/Academic Term/${encodeURIComponent(name)}`);
            ...
            notification.error({ message: 'Failed to load academic term details' });
        }
    };

    // ─── FILTER ──────────────────────────────────────────────────
    const filtered = data.filter(d => {
        if (searchId && !d.name.toLowerCase().includes(searchId.toLowerCase()) && !(d.academic_year || '').toLowerCase().includes(searchId.toLowerCase())) return false;
        return true;
    });
    ...
    const handleSave = async () => {
        if (!formData.academic_year || !formData.term_name || !formData.term_start_date || !formData.term_end_date) {
        ...
        try {
            ...
            if (editingRecord) {
                await API.put(`/api/resource/Academic Term/${encodeURIComponent(editingRecord.name)}`, payload);
                notification.success({ message: `"${editingRecord.name}" updated successfully!` });
            } else {
                await API.post('/api/resource/Academic Term', payload);
                notification.success({ message: `"${formData.term_name}" created successfully!` });
            }
            setView('list');
        } catch (err) {
    ...
    const handleDelete = async () => {
        ...
        try {
            await API.delete(`/api/resource/Academic Term/${encodeURIComponent(editingRecord.name)}`);
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="text-xl font-bold text-gray-900 tracking-tight">
                                {editingRecord ? editingRecord.name : 'New Academic Term'}
                            </span>
                            {!editingRecord ? (
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-6 space-y-6 max-w-3xl">
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                <label className="block text-[13px] text-gray-500 mb-1">Term Name <span className="text-red-400">*</span></label>
                                <input type="text"
                                    ...
                                    onChange={(e) => updateForm('term_name', e.target.value)}
                                    placeholder="e.g. Semester 1"
                                />
                            </div>
                            <div>
                                <label className="block text-[13px] text-gray-500 mb-1">Academic Year <span className="text-red-400">*</span></label>
                                <select
                                    className="w-full border border-gray-100 rounded px-3 py-1.5 text-sm focus:outline-none focus:border-blue-400 bg-white shadow-sm"
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                <label className="block text-[13px] text-gray-500 mb-1">Term Start Date <span className="text-red-400">*</span></label>
                                <input type="date"
                                    className="w-full border border-gray-100 rounded px-3 py-1.5 text-sm focus:outline-none focus:border-blue-400 bg-white shadow-sm"
                                    value={formData.term_start_date}
                                    onChange={(e) => updateForm('term_start_date', e.target.value)}
                                />
                            </div>
                            <div>
                                <label className="block text-[13px] text-gray-500 mb-1">Term End Date <span className="text-red-400">*</span></label>
                                <input type="date"
                                    className="w-full border border-gray-100 rounded px-3 py-1.5 text-sm focus:outline-none focus:border-blue-400 bg-white shadow-sm"
                                    value={formData.term_end_date}
                                    onChange={(e) => updateForm('term_end_date', e.target.value)}
                                />
                            </div>
                        </div>
                    </div>
    ...
    return (
        <div className="p-6">
            ...
            <div className="flex items-center gap-3 mb-4 flex-wrap">
                <input type="text" className="border border-gray-300 rounded px-3 py-2 text-sm w-64" placeholder="Search by term or year..." value={searchId} onChange={(e) => setSearchId(e.target.value)} />
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                {loading ? (
                    ...
                    <div className="text-center py-12 text-gray-500">
                        <p className="text-lg mb-2">No Academic Terms found</p>
                        <p className="text-sm">Click "+ Add Academic Term" to create one</p>
                    </div>
                ) : (
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b">
                            <tr>
                                <th className="text-left px-4 py-3 font-medium text-gray-600">Term Name</th>
                        ...
                        <tbody>
                            {filtered.map((row) => (
                                <tr key={row.name} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-gray-500">{row.academic_year || '-'}</td>
                                    <td className="px-4 py-3">
