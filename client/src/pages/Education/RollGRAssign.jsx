        ...
        for (let i = 0; i < studentsToUpdate.length; i++) {
            ...
            try {
                ...
                try {
                    ...
                    console.warn(`Failed to update final_admissions for ${student.id}:`, fbErr.message);
                }

                // 3. Update Firestore Registrations
                try {
                    let updatedReg = false;

                    // APPROACH 1: Best match — search by erp_student_id stored ON the Registration document
                    if (!updatedReg && student.erp_student_id) {
                        const q = query(collection(db, 'schooler_system/enquiry_management/registrations'), where('erp_student_id', '==', student.erp_student_id));
                        const snap = await getDocs(q);
                        if (!snap.empty) {
                            for (const regDoc of snap.docs) {
                                await updateDoc(doc(db, 'schooler_system/enquiry_management/registrations', regDoc.id), {
                                    roll_number: student.roll_number,
                                    gr_number: student.gr_number
                                });
                            }
                    ...
                    // APPROACH 2: Fallback — try the stored registrationId directly
                    if (!updatedReg && student.registrationId) {
                        try {
                            ...
                            // Registration document not found by this ID, try next approach
                        }
                    }

                    // APPROACH 3: Fallback — search by registrationNo field (the REG-XXXXXX code stored on Registration doc)
                    if (!updatedReg && student.registrationCode) {
                        const q = query(collection(db, 'schooler_system/enquiry_management/registrations'), where('registrationNo', '==', student.registrationCode));
                        const snap = await getDocs(q);
                        if (!snap.empty) {
                    ...
                    // APPROACH 4: Ultimate fallback — fetch ALL registrations and fuzzy-match by full_name
                    if (!updatedReg) {
                        const targetName = (student.full_name || student.student_full_name || student.first_name || '').toLowerCase().trim();
                        const allSnap = await getDocs(collection(db, 'schooler_system/enquiry_management/registrations'));
                        const matchedDoc = allSnap.docs.find(d => {
                            const data = d.data();
                            const regName = `${data.first_name || ''} ${data.middle_name || ''} ${data.last_name || ''}`.replace(/\s+/g, ' ').trim().toLowerCase();
                            const storedFull = (data.student_full_name || '').toLowerCase().trim();
                            return regName === targetName || storedFull === targetName || (targetName.length > 3 && (regName.includes(targetName) || targetName.includes(regName)));
                        });
                        if (matchedDoc) {
                            await updateDoc(doc(db, 'schooler_system/enquiry_management/registrations', matchedDoc.id), {
                                roll_number: student.roll_number,
                                gr_number: student.gr_number
                            });
                        }
                    ...
                    // Silently ignore registration update failures — student record was already updated
                }
