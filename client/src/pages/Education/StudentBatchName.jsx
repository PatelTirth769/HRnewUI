...
export default function StudentBatchName() {
    ...
    const defaultForm = { student_batch_name: '' };
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await API.get('/api/resource/Student Batch Name?fields=["name"]&limit_page_length=None&order_by=name asc');
            if (res.data.data) setData(res.data.data);
        } catch (err) {
            console.error('Fetch failed:', err);
            notification.error({ message: 'Failed to load Student Batch Names' });
    ...
    const fetchSingle = async (name) => {
        try {
            const res = await API.get(`/api/resource/Student Batch Name/${encodeURIComponent(name)}`);
            const d = res.data.data;
            setFormData({ student_batch_name: d.name || '' });
        } catch (err) {
            console.error('Fetch single failed:', err);
            notification.error({ message: 'Failed to load batch name details' });
        }
    };
    ...
    const handleSave = async () => {
        if (!formData.student_batch_name?.trim()) {
            notification.warning({ message: 'Batch Name is required' });
            return;
        }
        setSaving(true);
        try {
            const payload = { student_batch_name: formData.student_batch_name };
            if (editingRecord) {
                await API.put(`/api/resource/Student Batch Name/${encodeURIComponent(editingRecord.name)}`, payload);
                notification.success({ message: `"${editingRecord.name}" updated successfully!` });
            } else {
                await API.post('/api/resource/Student Batch Name', payload);
                notification.success({ message: `"${formData.student_batch_name}" created successfully!` });
            }
            setView('list');
        } catch (err) {
    ...
    const handleDelete = async () => {
        ...
        try {
            await API.delete(`/api/resource/Student Batch Name/${encodeURIComponent(editingRecord.name)}`);
            ...
            <div className="p-6 max-w-5xl mx-auto">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="text-xl font-bold text-gray-900 tracking-tight">
                                {editingRecord ? editingRecord.name : 'New Student Batch Name'}
                            </span>
                            {!editingRecord ? (
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-6 space-y-6 max-w-3xl">
                        <div>
                            <label className="block text-[13px] text-gray-500 mb-1">Batch Name <span className="text-red-400">*</span></label>
                            <input type="text"
                                ...
                                onChange={(e) => updateForm('student_batch_name', e.target.value)}
                                placeholder="e.g. 2024-28-Physics-A"
                            />
                        </div>
                    </div>
                </div>
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-semibold text-gray-800">Student Batch Name</h1>
            ...
            <div className="flex items-center gap-3 mb-4 flex-wrap">
                <input type="text" className="border border-gray-300 rounded px-3 py-2 text-sm w-64" placeholder="Search Batch Name..." value={searchId} onChange={(e) => setSearchId(e.target.value)} />
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                {loading ? (
                    ...
                    <div className="text-center py-12 text-gray-500">
                        <p className="text-lg mb-2">No Batch Names found</p>
                        <p className="text-sm">Click "+ Add Batch Name" to create one</p>
                    </div>
                ) : (
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b">
                            <tr>
                                <th className="text-left px-4 py-3 font-medium text-gray-600">Student Batch Name</th>
