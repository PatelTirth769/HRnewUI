    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                const domisile = allDocs.filter(d => d.type === 'Domisile & Character').length;

                setStats({ total, today, bonafide, trial, transfer, domisile });
        ...
        <div className="p-6 max-w-7xl mx-auto pb-40">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-gray-100 pb-5">
             ...
             <Row gutter={[20, 20]} className="mb-8">
                 ...
                 <Col xs={24} sm={4}>
                     <Card bordered={false} className="shadow-sm hover:shadow-md transition-shadow duration-200 border-l-4 border-purple-600">
                         <Statistic
                             title={<span className="text-gray-400 font-medium text-xs uppercase tracking-wider">Domisile Type</span>}
                             value={stats.domisile}
            ...
            <Row gutter={[24, 24]}>
                {/* Chart */}
                <Col xs={24} lg={16}>
                     <Card title={<span className="font-bold text-gray-800">Monthly Issuance Trend</span>} bordered={false} className="shadow-sm h-[500px]">
                 ...
                 {/* Actions & Help */}
                 <Col xs={24} lg={8}>
                     <Card title={<span className="font-bold text-gray-800">Quick Actions</span>} bordered={false} className="shadow-sm h-[500px] flex flex-col justify-between">
                         <div className="flex flex-col gap-2.5 w-full">
                             ...
                             <div 
                                 className="flex justify-between items-center p-2.5 rounded-xl border border-gray-100 hover:border-red-700 bg-gray-50 hover:bg-red-50/20 cursor-pointer transition-all duration-200 group"
                                 onClick={() => navigate('/certificates/domisile')}
                             >
                                 <div>
                                     <h4 className="font-bold text-gray-800 text-[13px]">Create Domisile & Character Cert.</h4>
                                     <p className="text-[11px] text-gray-500 mt-0.5">Select student and generate Domisile & Character PDF</p>
                                 </div>
                                 <ArrowRightOutlined className="text-gray-400 group-hover:text-red-700 transition-colors" />
                             </div>
