...
export default function SalesOrderAnalysis() {
    ...
    const runReport = async () => {
        ...
        try {
            ...
            // Usually report columns: billed_amount, amount_to_bill
    ...
    return (
        <div className="p-6 max-w-[1400px] mx-auto text-gray-800">
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-[24px] font-bold text-gray-900 tracking-tight">Sales Order Analysis</h1>
                    ...
                    <div className="grid grid-cols-6 gap-4 mb-4">
                        ...
                        <div>
                            <Select
                                ...
                                allowClear
                                showSearch
                                value={filters.sales_order || undefined}
                                onChange={v => handleFilterChange('sales_order', v)}
                                options={masters.sales_orders.map(s => ({ value: s.name, label: s.name }))}
                            />
                        </div>
                        <div>
                            <Select
                        ...
                        <div>
                            <Select
                                ...
                                options={[{value:'To Bill',label:'To Bill'},{value:'To Deliver',label:'To Deliver'},{value:'Completed',label:'Completed'}]}
                            />
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Checkbox checked={!!filters.group_by_so} onChange={e => handleFilterChange('group_by_so', e.target.checked ? 1 : 0)} />
                        <span className="text-[12px] text-gray-500 font-medium">Group by Sales Order</span>
                    </div>
                </div>
                ...
                <div className="min-h-[500px] bg-[#fbfcfd] border-t border-gray-100 p-8 flex flex-col">
                    {loading ? (
                        <div className="flex-1 flex flex-col items-center justify-center gap-3">
                            <Spin size="large" />
                            <span className="text-sm text-gray-400 font-medium">Analyzing Sales Orders...</span>
                        </div>
                    ) : reportData.length > 0 ? (
                        <>
                            {/* Chart Area */}
                            ...
                            <div className="w-full overflow-x-auto">
                                <Table 
                                    ...
                                    summary={(pageData) => {
                                        let totals = {};
                                        columns.forEach(col => {
                                            if (['Qty', 'Delivered Qty', 'Qty to Deliver', 'Billed Amount', 'Qty to Bill'].includes(col.title)) {
