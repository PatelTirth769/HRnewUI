...
export default function RecommendedReorderLevel() {
    ...
    const fetchMasters = async () => {
        try {
            const [igRes, brandRes] = await Promise.all([
                API.get('/api/resource/Item Group?fields=["name"]&limit_page_length=200'),
    ...
    const runReport = async () => {
        if (!filters.item_group) {
            notification.warning({ message: 'Filter Required', description: 'Please select an Item Group to continue.' });
            return;
        }
        ...
        <div className="p-6 max-w-[1400px] mx-auto text-gray-800">
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-[24px] font-bold text-gray-900 tracking-tight whitespace-nowrap overflow-hidden text-ellipsis mr-4">
                    Itemwise Recommended Reorder L...
                </h1>
                    ...
                    <div className="grid grid-cols-4 gap-4 max-w-[1000px]">
                        ...
                        <div>
                            <Select
                                ...
                                options={masters.item_groups.map(ig => ({ value: ig.name, label: ig.name }))}
                            />
                        </div>
                        <div>
                            <Select
                ...
                <div className="min-h-[400px] bg-[#fbfcfd] border-t border-gray-100 flex flex-col items-center justify-center p-8 relative">
                    {loading ? (
                        <div className="flex flex-col items-center gap-3">
                            <Spin size="large" />
                            <span className="text-sm text-gray-400 font-medium">Calculating Reorder Levels...</span>
                        </div>
                    ) : reportData.length > 0 ? (
