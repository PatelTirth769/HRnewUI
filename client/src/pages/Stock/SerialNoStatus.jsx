...
import { FiRefreshCw, FiChevronLeft, FiChevronRight, FiPrinter, FiMoreHorizontal, FiChevronUp, FiChevronDown, FiCopy, FiEdit2 } from 'react-icons/fi';
        ...
        <table className="w-full text-left text-[13px]">
            <thead>
                <tr className="bg-gray-50">
                    ...
                    {columns.map((col) => (
                        <th key={col.key} className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider border-b border-gray-200">
...
/* ════════════════════════════════════════════════════
   MAIN COMPONENT
   ════════════════════════════════════════════════════ */
export default function SerialNoStatus() {
    ...
    const fetchReportMeta = async () => {
        setLoading(true);
        try {
            const res = await API.get(`/api/resource/Report/${encodeURIComponent('Serial No Status')}`);
            ...
            setClientCode(JSON.stringify({
                ...
                columns: [
                    ["name", "Serial No"], ["item_code", "Serial No"], ["status", "Serial No"],
                    ["item_name", "Serial No"], ["description", "Serial No"],
                    ["item_group", "Serial No"], ["brand", "Serial No"], ["warehouse", "Serial No"]
                ]
            }, null, 2));
            setRoleRows([
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await API.put(`/api/resource/Report/${encodeURIComponent('Serial No Status')}`, payload);
    ...
    const handleToggleDisabled = async () => {
        ...
        setDirty(true);
        try {
            await API.put(`/api/resource/Report/${encodeURIComponent('Serial No Status')}`, { disabled: newVal ? 1 : 0 });
    ...
    const handleShowReport = async () => {
        ...
        try {
            const fields = encodeURIComponent(
                '["name","serial_no","item_code","item_name","status","company","warehouse","maintenance_status","amc_expiry_date","brand","item_group","description"]'
            );
            let url = `/api/resource/Serial No?fields=${fields}&limit_page_length=500&order_by=modified desc`;
            const apiFilters = [];
            if (reportFilters.serial_no) apiFilters.push(`["name","=","${reportFilters.serial_no}"]`);
            if (reportFilters.item_code) apiFilters.push(`["item_code","=","${reportFilters.item_code}"]`);
            if (reportFilters.status) apiFilters.push(`["status","=","${reportFilters.status}"]`);
            if (reportFilters.company) apiFilters.push(`["company","=","${reportFilters.company}"]`);
            ...
            <div className="text-gray-800 h-full flex flex-col">
                ...
                <div className="px-6 py-3 bg-white border-b border-gray-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-lg font-bold text-gray-900">Serial No Status</h1>
                        {dirty && (
                                    ...
                                    <table className="w-full text-left text-[13px] bg-white">
                                        <thead className="sticky top-0 z-10">
                                            <tr className="bg-gray-50 border-b border-gray-200">
                                                ...
                                                <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Brand</th>
                                                <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Item Group</th>
                                            </tr>
                                        </thead>
                                                    ...
                                                    <tr key={row.name} className={`transition-colors cursor-pointer ${isSelected ? 'bg-blue-50/60' : 'hover:bg-gray-50'}`}
                                                        ...
                                                        <td className="px-4 py-3">
                                                            <span className={`text-[11px] px-2 py-0.5 rounded font-semibold ${
                                                                ...
                                                                row.status === 'Inactive' ? 'bg-gray-100 text-gray-500' :
                                                        ...
                                                        <td className="px-4 py-3 text-gray-600">{row.item_group || '-'}</td>
                                                    </tr>
                                                );
                                            })}
                                        </tbody>
        ...
        <div className="p-6 max-w-[1200px] mx-auto pb-24 text-gray-800">
            <Spin spinning={loading}>
                ...
                <div className="flex items-center gap-1.5 text-[12px] text-gray-400 mb-4 font-medium">
                    ...
                    <span>›</span>
                    <span className="text-gray-600">Serial No Status</span>
                </div>
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-xl font-bold text-gray-900">Serial No Status</h1>
