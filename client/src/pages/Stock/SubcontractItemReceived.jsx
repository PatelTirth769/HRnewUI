...
export default function SubcontractItemReceived() {
    ...
    const runReport = async () => {
        if (!filters.supplier) {
            notification.warning({ message: 'Filter Required', description: 'Please select a Supplier to view items pending receipt.' });
            return;
        }
                ...
                <div className="min-h-[400px] bg-[#fbfcfd] border-t border-gray-100 flex flex-col items-center justify-center p-8 relative">
                    {loading ? (
                        <div className="flex flex-col items-center gap-3">
                            <Spin size="large" />
                            <span className="text-sm text-gray-400 font-medium">Tracking Shipments...</span>
                        </div>
                    ) : reportData.length > 0 ? (
