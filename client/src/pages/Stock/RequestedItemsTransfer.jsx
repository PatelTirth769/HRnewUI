...
export default function RequestedItemsTransfer() {
    ...
    const fetchMasters = async () => {
        try {
            const compRes = await API.get('/api/resource/Company?fields=["name"]&limit_page_length=50');
            setMasters({
    ...
    return (
        <div className="p-6 max-w-[1400px] mx-auto text-gray-800">
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-[24px] font-bold text-gray-900 tracking-tight">Requested Items To Be Transferred</h1>
                ...
                <div className="min-h-[400px] bg-[#fbfcfd] border-t border-gray-100 flex flex-col items-center justify-center p-8 relative">
                    {loading ? (
                        <div className="flex flex-col items-center gap-3">
                            <Spin size="large" />
                            <span className="text-sm text-gray-400 font-medium">Fetching Data...</span>
                        </div>
                    ) : reportData.length > 0 ? (
