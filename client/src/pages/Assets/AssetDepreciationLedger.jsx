...
const SelectField = ({ label, value, options, onChange, disabled = false }) => (
    <div>
        ...
        <select className={`w-full border border-gray-100 rounded bg-[#fcfcfc] px-3 py-2 text-[13px] focus:outline-none ${disabled ? 'bg-gray-50 pointer-events-none' : 'focus:border-blue-400 hover:border-gray-300 transition-colors'}`}
...
const AssetDepreciationLedger = () => {
    ...
    const fetchMasters = async () => {
        try {
            const [cR, aR, catR, fbR, ccR] = await Promise.all([
                ...
                API.get('/api/resource/Asset Category?fields=["name"]&limit_page_length=None').catch(() => ({ data: { data: [] } })),
            ...
            setCategories((catR.data.data || []).map(d => d.name));
    ...
    const runReport = async () => {
        try {
            setLoading(true);
            const params = new URLSearchParams({ report_name: 'Asset Depreciation Ledger' });
            const filterObj = {};
            ...
            if (filters.asset_category) filterObj.asset_category = filters.asset_category;
            if (filters.cost_center) filterObj.cost_center = filters.cost_center;
            if (filters.finance_book) filterObj.finance_book = filters.finance_book;
            filterObj.include_default_book_assets = filters.include_default_book_assets;
    ...
    const handleRefresh = () => runReport();

    return (
        <div className="p-6 max-w-7xl mx-auto font-sans">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-900">Asset Depreciation Ledger</h1>
                <div className="flex items-center gap-2">
                    <span className="text-[13px] text-gray-500">Actions</span>
                    <button onClick={handleRefresh} className="p-2 border border-gray-200 rounded hover:bg-gray-50 transition-colors" title="Refresh">
            ...
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 mb-6">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-3">
                    ...
                    <SelectField label="Asset Category" value={filters.asset_category} options={categories} onChange={(v) => setFilters({ ...filters, asset_category: v })} />
                    <SelectField label="Cost Center" value={filters.cost_center} options={costCenters} onChange={(v) => setFilters({ ...filters, cost_center: v })} />
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
                    <SelectField label="Finance Book" value={filters.finance_book} options={financeBooks} onChange={(v) => setFilters({ ...filters, finance_book: v })} />
                    <div className="flex items-center gap-2 pt-5">
                        <input type="checkbox" id="include_default" className="w-3.5 h-3.5 rounded border-gray-300 text-blue-600"
                            checked={!!filters.include_default_book_assets} onChange={(e) => setFilters({ ...filters, include_default_book_assets: e.target.checked ? 1 : 0 })} />
                        <label htmlFor="include_default" className="text-[13px] text-gray-600 cursor-pointer">Include Default FB Assets</label>
                    </div>
                    <div className="pt-5">
...
export default AssetDepreciationLedger;
