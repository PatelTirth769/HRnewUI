...
const groupByOptions = ['Asset Category', 'Asset', 'Finance Book'];

const AssetDepreciationsBalances = () => {
    ...
    const fiscalStart = new Date(today.getFullYear(), 3, 1); // April 1 of current year
    if (fiscalStart > today) fiscalStart.setFullYear(fiscalStart.getFullYear() - 1);

    const [filters, setFilters] = useState({
        company: '',
        from_date: fiscalStart.toISOString().split('T')[0],
    ...
    const fetchMasters = async () => {
        try {
            const [cR, catR, fbR] = await Promise.all([
    ...
    const runReport = async () => {
        try {
            setLoading(true);
            const params = new URLSearchParams({ report_name: 'Asset Depreciations and Balances' });
            const filterObj = {};
            ...
            if (filters.group_by) filterObj.group_by = filters.group_by;
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto font-sans">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-900">Asset Depreciations and Balances</h1>
            ...
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 mb-6">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
                    ...
                    <SelectField label="Group By" value={filters.group_by} options={groupByOptions} onChange={(v) => setFilters({ ...filters, group_by: v })} />
...
export default AssetDepreciationsBalances;
