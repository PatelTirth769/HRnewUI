...
const RetentionBonus = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            console.error('Fetch retention bonus failed:', err);
    ...
    const handleEdit = async (record) => {
        setLoading(true);
        try {
            const res = await API.get(`/api/resource/Retention Bonus/${encodeURIComponent(record.name)}`);
            if (res.data.data) {
    ...
    const handleSave = async (submit = false) => {
        ...
        if (!formData.bonus_payment_date) { notification.warning({ message: 'Bonus Payment Date is required' }); return; }
        if (formData.bonus_amount === '' || formData.bonus_amount === null) { notification.warning({ message: 'Bonus Amount is required' }); return; }
        ...
        try {
            const payload = {
                ...
                bonus_payment_date: formData.bonus_payment_date,
                bonus_amount: parseFloat(formData.bonus_amount),
                docstatus: submit ? 1 : formData.docstatus
            };

            if (editingRecord) {
                await API.put(`/api/resource/Retention Bonus/${encodeURIComponent(editingRecord.name)}`, payload);
                notification.success({ message: `"${editingRecord.name}" ${submit ? 'submitted' : 'updated'} successfully!` });
            } else {
                const res = await API.post('/api/resource/Retention Bonus', payload);
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-[#F8FAFC] text-gray-500 border-b border-gray-100 text-[13px] sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 font-medium cursor-pointer hover:bg-gray-100">Bonus Payment Date</th>
                                    <th className="px-4 py-3 font-medium cursor-pointer hover:bg-gray-100">Bonus Amount</th>
                                ...
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan="9" className="text-center py-12">
                                            ...
                                            <p className="text-gray-500 text-base">No Retention Bonus Found</p>
                                        </td>
                                    </tr>
                                ) : (
                                    data.map((row) => {
                                        ...
                                        return (
                                            <tr key={row.name} className="hover:bg-gray-50/80 cursor-pointer transition-colors" onClick={() => handleEdit(row)}>
                                                ...
                                                <td className="px-4 py-2.5 text-gray-600">{row.bonus_payment_date}</td>
                                                <td className="px-4 py-2.5 text-gray-900">{formatINR(row.bonus_amount)}</td>
            ...
            <div className="p-6 max-w-7xl mx-auto font-sans">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="text-xl font-bold text-gray-900 tracking-tight">
                                {editingRecord ? editingRecord.name : 'New Retention Bonus'}
                            </span>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-6 space-y-8">
                        {/* Section: Employee */}
                        <div>
                            <h3 className="font-semibold text-gray-800 mb-6 text-sm">Employee</h3>
                        ...
                        {/* Section: Bonus */}
                        <div>
                            <h3 className="font-semibold text-gray-800 mb-6 text-sm">Bonus</h3>
                            <div className="grid grid-cols-2 gap-x-12">
                                <div className="space-y-6">
                                    <SelectField
                                    ...
                                    <InputField
                                        ...
                                        onChange={(v) => setFormData({ ...formData, bonus_amount: v })}
                                        disabled={isSubmitted}
                                    />
                                </div>
                                <div className="space-y-6">
                                    <InputField
                                        ...
                                        type="date"
                                        required
                                        value={formData.bonus_payment_date}
                                        onChange={(v) => setFormData({ ...formData, bonus_payment_date: v })}
                                        disabled={isSubmitted}
                                    />
                                </div>
                            </div>
