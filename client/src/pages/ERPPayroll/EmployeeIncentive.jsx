...
const EmployeeIncentive = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            console.error('Fetch employee incentive failed:', err);
    ...
    const handleEdit = async (record) => {
        setLoading(true);
        try {
            const res = await API.get(`/api/resource/Employee Incentive/${encodeURIComponent(record.name)}`);
            if (res.data.data) {
    ...
    const handleSave = async (submit = false) => {
        ...
        if (formData.incentive_amount === '' || formData.incentive_amount === null) { notification.warning({ message: 'Incentive Amount is required' }); return; }
        ...
        try {
            ...
            if (editingRecord) {
                await API.put(`/api/resource/Employee Incentive/${encodeURIComponent(editingRecord.name)}`, payload);
                notification.success({ message: `"${editingRecord.name}" ${submit ? 'submitted' : 'updated'} successfully!` });
            } else {
                const res = await API.post('/api/resource/Employee Incentive', payload);
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button className="px-4 py-2 bg-gray-900 text-white rounded-md text-sm font-medium hover:bg-gray-800 flex items-center gap-1.5 shadow-sm" onClick={handleNew}>
                            <span>+</span> Add Employee Incentive
                        </button>
                    </div>
                </div>
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-[#F8FAFC] text-gray-500 border-b border-gray-100 text-[13px] sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 font-medium cursor-pointer hover:bg-gray-100">Incentive Amount</th>
                                ...
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan="9" className="text-center py-12">
                                            ...
                                            <p className="text-gray-500 text-base">No Employee Incentives Found</p>
                                        </td>
                                    </tr>
                                ) : (
                                    data.map((row) => {
                                        ...
                                        return (
                                            <tr key={row.name} className="hover:bg-gray-50/80 cursor-pointer transition-colors" onClick={() => handleEdit(row)}>
                                                ...
                                                <td className="px-4 py-2.5 text-gray-900">{formatINR(row.incentive_amount)}</td>
            ...
            <div className="p-6 max-w-7xl mx-auto font-sans">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="text-xl font-bold text-gray-900 tracking-tight">
                                {editingRecord ? editingRecord.name : 'New Employee Incentive'}
                            </span>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-6 space-y-8">
                        ...
                        {/* Section: Incentive */}
                        <div>
                            <h3 className="font-semibold text-gray-800 mb-6 text-sm">Incentive</h3>
                            <div className="grid grid-cols-2 gap-x-12">
                                <div className="space-y-6">
                                    <SelectField
                                ...
                                <div className="space-y-6">
                                    <InputField
                                        ...
                                        onChange={(v) => setFormData({ ...formData, payroll_date: v })}
                                        disabled={isSubmitted}
                                    />
                                    <InputField
                                        ...
                                        onChange={(v) => setFormData({ ...formData, incentive_amount: v })}
                                        disabled={isSubmitted}
                                    />
                                </div>
                            </div>
