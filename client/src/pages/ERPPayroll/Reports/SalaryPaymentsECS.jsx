...
export default function SalaryPaymentsECS() {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const rows = reportResult.map((row, idx) => {
                ...
                const employee = rowObj.employee || rowObj.Employee || '';
                if (
                    branchVal.toString().toLowerCase().includes('total') ||
                    empName.toString().toLowerCase().includes('total') ||
                    employee.toString().toLowerCase().includes('total')
                ) {
                    rowObj.isTotal = true;
                }
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Salary Payments ECS");
        XLSX.writeFile(wb, `Salary_Payments_ECS_${months.find(m => m.value === month)?.label}_${year}.xlsx`);
    };
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)', padding: '24px', background: '#f9fafb', overflow: 'hidden', fontFamily: 'sans-serif' }}>
            ...
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold text-gray-900 m-0 truncate">Salary Payments via ECS</h1>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, border: '1px solid #e5e7eb', borderRadius: '6px', background: '#fff', boxShadow: '0 1px 2px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                ...
                {/* Filters Row - matching ERPNext: Company | Month | Year | Department | Branch | Type */}
                <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-100 flex-wrap shrink-0">
                    ...
                    <select className="bg-[#f0f1f3] border-none rounded px-3 py-[3px] text-[13px] text-gray-700 outline-none w-[100px] h-[26px] hover:bg-[#e4e6ea] transition-colors cursor-pointer appearance-none"
                        value={type} onChange={e => { setType(e.target.value); handleGenerate(); }}>
                ...
                {/* Main Content (Table) — scroll container; width:0 + min-width:100% is the flex-child containment trick */}
