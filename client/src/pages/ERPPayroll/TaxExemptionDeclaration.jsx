...
const TaxExemptionDeclaration = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            console.error('Fetch tax exemption declaration failed:', err);
    ...
    const handleNew = () => {
        ...
        setFormData({ ...defaultForm, company: companies[0] || 'Preeshe Consultancy Services', declarations: [] });
        setView('form');
    };
    ...
    const addDeclaration = () => {
        setFormData(prev => ({
            ...
            declarations: [
                ...(prev.declarations || []),
                { exemption_sub_category: '', exemption_category: '', amount: 0, max_amount: 0 }
            ]
        }));
    };
    ...
    const removeDeclaration = (index) => {
        setFormData(prev => {
            const newDecls = [...(prev.declarations || [])];
            newDecls.splice(index, 1);
            return { ...prev, declarations: newDecls };
        });
    };

    const updateDeclaration = (index, field, value) => {
        setFormData(prev => {
            const newDecls = [...(prev.declarations || [])];
            newDecls[index] = { ...newDecls[index], [field]: value };
            return { ...prev, declarations: newDecls };
        });
    };
    ...
    const handleSave = async (submit = false) => {
        ...
        try {
            ...
            if (editingRecord) {
                ...
                const res = await API.post('/api/resource/Employee Tax Exemption Declaration', payload);
                notification.success({ message: `"${res.data?.data?.name || 'Declaration'}" created successfully!` });
            }
            setView('list');
            fetchData();
        } catch (err) {
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-[#F8FAFC] text-gray-500 border-b border-gray-100 text-[13px] sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 font-medium cursor-pointer hover:bg-gray-100">Declared Amount</th>
                                ...
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan="8" className="text-center py-12">
                                            ...
                                            <p className="text-gray-500 text-base">No Declarations Found</p>
                                        </td>
                                    </tr>
                                ) : (
                                    data.map((row) => {
                                        ...
                                        return (
                                            <tr key={row.name} className="hover:bg-gray-50/80 cursor-pointer transition-colors" onClick={() => handleEdit(row)}>
                                                ...
                                                <td className="px-4 py-2.5 text-gray-900">{formatINR(row.total_declared_amount || row.total_exemption_amount)}</td>
            ...
            <div className="p-6 max-w-7xl mx-auto font-sans">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="text-xl font-bold text-gray-900 tracking-tight">
                                {editingRecord ? editingRecord.name : 'New Tax Exemption Declaration'}
                            </span>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-6 space-y-8">
                        ...
                        {/* Declarations Subtable */}
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="font-semibold text-gray-800 text-sm">Declarations</h3>
                            </div>
                                ...
                                <table className="w-full text-left text-sm whitespace-nowrap">
                                    <thead className="bg-[#F8FAFC] text-gray-500 border-b border-gray-100 text-[13px]">
                                        <tr>
                                            ...
                                            <th className="px-4 py-2 font-medium w-1/3">Exemption Sub Category</th>
                                            <th className="px-4 py-2 font-medium w-1/3">Exemption Category</th>
                                            <th className="px-4 py-2 font-medium">Declared Amount</th>
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                            ...
                                            <tr key={index} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-4 py-2">
                                                    <input
                                                        ...
                                                        value={dec.exemption_sub_category || ''}
                                                        onChange={(e) => updateDeclaration(index, 'exemption_sub_category', e.target.value)}
                                                        placeholder="Exemption Sub Category"
                                                ...
                                                <td className="px-4 py-2">
                                                    <input
                                                        ...
                                                        value={dec.exemption_category || ''}
                                                        onChange={(e) => updateDeclaration(index, 'exemption_category', e.target.value)}
                                                ...
                                                <td className="px-4 py-2">
                                                    <input
                                                        ...
                                                        value={dec.amount}
                                                        onChange={(e) => updateDeclaration(index, 'amount', e.target.value)}
                                                        placeholder="0.00"
                                                        readOnly={isSubmitted}
                                                    />
                                                </td>
                                                <td className="px-4 py-2">
                                                    <input
                                                        ...
                                                        value={dec.max_amount}
                                                        onChange={(e) => updateDeclaration(index, 'max_amount', e.target.value)}
                                                        placeholder="0.00"
                                                        readOnly={isSubmitted}
                                                    />
                                                </td>
                                        ...
                                        {(!formData.declarations || formData.declarations.length === 0) && (
                                            <tr>
                                                <td colSpan={isSubmitted ? 5 : 6} className="text-center py-4 text-[13px] text-gray-500">
