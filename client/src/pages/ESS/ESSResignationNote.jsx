...
export default function ESSResignationNote({ employeeData }) {
    ...
    const fetchHistory = async () => {
        setLoading(true);
        try {
            const res = await API.get(`/api/resource/Employee Resignation`, {
                params: {
                    fields: JSON.stringify(["name", "resignation_date", "reason", "docstatus", "status"]),
            ...
            console.warn("Could not fetch Resignation history. Ensure 'Employee Resignation' DocType is accessible.", err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleApply = async (values) => {
        ...
        try {
            ...
            await API.post('/api/resource/Employee Resignation', payload);
            notification.success({ message: 'Resignation submitted successfully' });
            form.resetFields();
            setIsModalOpen(false);
            fetchHistory();
        } catch (err) {
            console.error('Failed to submit resignation:', err);
            let errMsg = "Failed to submit resignation";
    ...
    const handleDelete = async (recordName) => {
        try {
            await API.delete(`/api/resource/Employee Resignation/${recordName}`);
    ...
    const columns = [
        ...
        { title: 'Resignation Date', dataIndex: 'resignation_date', key: 'resignation_date', render: (d) => dayjs(d).format('DD MMM YYYY') },
        ...
        { title: 'Status', key: 'display_status', render: (_, record) => {
            ...
            const statusLabel = record.status || (isCancelled ? 'Cancelled' : isDraft ? 'Draft' : 'Submitted');
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-gray-800 m-0">My Resignation Notes</h2>
                <Button 
                    type="primary" 
            ...
            >
                <Form form={form} layout="vertical" onFinish={handleApply} className="mt-4">
                    <Row gutter={16}>
                        <Col span={24}>
                            <Form.Item name="resignation_date" label="Proposed Last Working Day" rules={[{ required: true, message: 'Please select a date' }]}>
                    ...
                    <Row gutter={16}>
                        <Col span={24}>
                            <Form.Item name="reason" label="Reason for Resignation" rules={[{ required: true, message: 'Please provide a reason' }]}>
                                <TextArea rows={4} placeholder="Please provide your reason for leaving" />
                            </Form.Item>
                        </Col>
                    </Row>
