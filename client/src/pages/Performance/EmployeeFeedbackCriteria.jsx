...
const RESOURCE = 'Employee Feedback Criteria';
const API_BASE = `/api/resource/${encodeURIComponent(RESOURCE)}`;

export default function EmployeeFeedbackCriteria() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await API.get(`${API_BASE}?fields=["name","criteria","modified"]&limit_page_length=None&order_by=modified desc`);
            if (res.data.data) setData(res.data.data);
        } catch (err) {
            console.error('Fetch failed:', err);
            notification.error({ message: `Failed to load ${RESOURCE}` });
        } finally { setLoading(false); }
    };

    useEffect(() => { if (view === 'list') fetchData(); }, [view]);
    ...
    const fetchSingle = async (name) => {
        try {
            ...
            setFormData({ criteria: d.criteria || d.name || '' });
        } catch (err) {
    ...
    const handleSave = async () => {
        if (!formData.criteria?.trim()) {
            notification.warning({ message: 'Criteria is required' });
            return;
        }
        ...
        try {
            if (editingRecord) {
                await API.put(`${API_BASE}/${encodeURIComponent(editingRecord.name)}`, {
                ...
                notification.success({ message: `"${formData.criteria}" created successfully!` });
            }
            setView('list');
            fetchData();
        } catch (err) {
    ...
    const handleDelete = async () => {
        ...
        try {
            await API.delete(`${API_BASE}/${encodeURIComponent(editingRecord.name)}`);
            ...
            <div className="p-6 max-w-5xl mx-auto">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="text-xl font-bold text-gray-900 tracking-tight">
                                {editingRecord ? editingRecord.name : `New ${RESOURCE}`}
                            </span>
                            {!editingRecord ? (
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-6 space-y-6 max-w-3xl">
                        <div>
                            <label className="block text-[13px] text-gray-500 mb-1">Criteria <span className="text-red-400">*</span></label>
                            {editingRecord ? (
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-semibold text-gray-800">{RESOURCE}</h1>
                        ...
                        <tbody>
                            {filtered.map((row, i) => (
                                <tr key={row.name} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3">{row.criteria || row.name}</td>
                                    ...
                                    <td className="px-4 py-3">
                                        ...
                                        <button className="text-red-600 hover:underline text-xs" onClick={() => {
                                            if (window.confirm(`Are you sure you want to delete "${row.name}"?`)) {
                                                API.delete(`${API_BASE}/${encodeURIComponent(row.name)}`)
                                                    .then(() => {
