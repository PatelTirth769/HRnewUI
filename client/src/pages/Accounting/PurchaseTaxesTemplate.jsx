...
const PurchaseTaxesTemplate = () => {
    ...
    const fetchTemplates = async () => {
        try {
            setLoadingList(true);
            const response = await API.get('/api/resource/Purchase Taxes and Charges Template?fields=["name","title","company","is_default","disabled"]&limit_page_length=None');
    ...
    const fetchTemplate = async (id) => {
        setLoadingForm(true);
        try {
            const res = await API.get(`/api/resource/Purchase Taxes and Charges Template/${encodeURIComponent(id)}`);
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingRecord) {
                await API.put(`/api/resource/Purchase Taxes and Charges Template/${encodeURIComponent(editingRecord)}`, form);
                notification.success({ message: 'Template updated.' });
            } else {
                await API.post('/api/resource/Purchase Taxes and Charges Template', form);
    ...
    const handleDelete = async () => {
        ...
        try {
            await API.delete(`/api/resource/Purchase Taxes and Charges Template/${encodeURIComponent(editingRecord)}`);
            ...
            <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center">
                        ...
                        <h1 className="text-2xl font-semibold text-gray-800">Purchase Taxes Templates</h1>
                    </div>
                ...
                <div className="mb-4 text-sm text-gray-500 bg-blue-50/50 p-3 rounded border border-blue-100 italic">
                    Configure purchase tax and charge templates for purchase orders and invoices.
                </div>
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 border-b">
                            <tr>
                                <th className="px-6 py-3 font-medium text-gray-600 flex items-center gap-2 uppercase tracking-tight text-[11px]">Title</th>
                                <th className="px-6 py-3 font-medium text-gray-600 uppercase tracking-tight text-[11px]">Company</th>
                                <th className="px-6 py-3 font-medium text-gray-600 text-center uppercase tracking-tight text-[11px]">Default</th>
                                ...
                                filtered.map((row) => (
                                    <tr key={row.name} className="border-b hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-3 text-center">
                                            {row.is_default ? <span className="bg-green-50 text-green-600 px-2.5 py-1 rounded-md uppercase text-[10px] font-bold tracking-wider border border-green-100">Default</span> : '-'}
                                        </td>
                                        <td className="px-6 py-3 text-center">
                                            <span className={`px-2.5 py-1 rounded-md text-[10px] uppercase font-bold tracking-wider border ${!row.disabled ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>
            ...
            <div className="flex justify-between items-start mb-6 pb-4 border-b border-gray-200">
                <div className="flex items-center gap-2">
                    <span className="text-xl font-bold text-gray-900 leading-tight">
                        {editingRecord ? editingRecord : 'New Purchase Taxes and Charges Template'}
                    </span>
                    {!editingRecord && (
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-8 space-y-10 text-gray-800">
                ...
                <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                    <div>
                        ...
                        <input className={inputStyle} value={form.title} onChange={e => updateField('title', e.target.value)} placeholder="e.g. Standard Purchase Taxes" />
                    </div>
                    <div>
                    ...
                    <div className="space-y-4">
                        <label className="flex items-center gap-2 cursor-pointer text-[13px] text-gray-800 font-medium group">
                            <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 transition cursor-pointer" checked={!!form.is_default} onChange={e => updateField('is_default', e.target.checked ? 1 : 0)} />
                            <span className="group-hover:text-blue-600 transition">Default</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer text-[13px] text-gray-800 font-medium group">
                            ...
                            <span className="group-hover:text-red-500 transition">Disabled</span>
                        </label>
                    </div>
                    <div>
                ...
                <div className="pt-8 border-t border-gray-100">
                    <h3 className="font-bold text-gray-800 text-[15px] mb-2 tracking-tight">Purchase Taxes and Charges</h3>
                    <div className="bg-gray-50 border border-gray-200 rounded-lg overflow-hidden mb-4 shadow-sm">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="bg-white divide-y divide-gray-100">
                                {form.taxes.length === 0 ? (
                                    <tr><td colSpan="7" className="px-4 py-16 text-center text-gray-400 italic flex flex-col items-center gap-3 bg-white/50">
                                    ...
                                    form.taxes.map((row, idx) => (
                                        <tr key={row.id}>
                                            ...
                                            <td className="px-4 py-3">
                                                <select className="w-full border-none focus:ring-0 text-sm py-1 bg-transparent font-medium" value={row.charge_type} onChange={e => updateRow(row.id, 'charge_type', e.target.value)}>
                                                    {chargeTypes.map(t => <option key={t} value={t}>{t}</option>)}
                                                </select>
                                            </td>
                                            <td className="px-4 py-3">
                                                <select className="w-full border-none focus:ring-0 text-sm py-1 bg-transparent font-medium" value={row.account_head} onChange={e => updateRow(row.id, 'account_head', e.target.value)}>
                                            ...
                                            <td className="px-4 py-3 text-center">
                                                <button className="text-gray-300 hover:text-red-500 transition-colors duration-200" onClick={() => removeRow(row.id)}>
                        ...
                        <div className="p-3 bg-white border-t border-gray-200">
                            <button className="px-4 py-1.5 bg-gray-50 border border-gray-200 rounded text-[13px] font-bold text-gray-600 hover:bg-gray-100 transition-all duration-200 shadow-sm" onClick={addRow}>
                                Add Row
                            </button>
                        </div>
                    </div>
...
export default PurchaseTaxesTemplate;
