...
function ManageBacklogNorms() {
  ...
  useEffect(() => {
    ...
    const fetchBacklogNorms = async () => {
      try {
        ...
        const response = await axios.get('/api/education/backlog-norms');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        await axios.put(`/api/education/backlog-norms/${currentId}`, formData);
        toast.success('Backlog norm updated successfully');
      } else {
        // Create new backlog norm
        await axios.post('/api/education/backlog-norms', formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this backlog norm?')) {
      try {
        await axios.delete(`/api/education/backlog-norms/${id}`);
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      setLoading(true);
      await axios.post('/api/education/backlog-norms/upload', formData, {
        headers: {
  ...
  const handleExcelDownload = async () => {
    try {
      const response = await axios.get('/api/education/backlog-norms/download', {
        responseType: 'blob'
      });
