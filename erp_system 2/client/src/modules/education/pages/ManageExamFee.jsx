...
function ManageExamFee() {
  ...
  useEffect(() => {
    ...
    const fetchExamFees = async () => {
      try {
        ...
        const response = await axios.get('/api/education/exam-fees');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        await axios.put(`/api/education/exam-fees/${currentId}`, formData);
        toast.success('Exam fee updated successfully');
      } else {
        // Create new exam fee
        await axios.post('/api/education/exam-fees', formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this exam fee?')) {
      try {
        await axios.delete(`/api/education/exam-fees/${id}`);
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      setLoading(true);
      await axios.post('/api/education/exam-fees/upload', formData, {
        headers: {
  ...
  const handleExcelDownload = async () => {
    try {
      const response = await axios.get('/api/education/exam-fees/download', {
        responseType: 'blob'
      });
