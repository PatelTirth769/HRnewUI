...
const PreExamReports = () => {
  ...
  const generateReport = async (type = 'download') => {
    ...
    try {
      ...
      if (type === 'preview') {
        const response = await axios.get(`/api/education/reports/pre-exam/preview?${params.toString()}`);
        setReportData(response.data);
        setShowPreview(true);
      } else {
        const response = await axios.get(`/api/education/reports/pre-exam/download?${params.toString()}`, {
          responseType: 'blob'
        });
