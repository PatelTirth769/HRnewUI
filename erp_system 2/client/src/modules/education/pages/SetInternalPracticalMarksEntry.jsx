...
function SetInternalPracticalMarksEntry() {
  ...
  useEffect(() => {
    const fetchMarksEntries = async () => {
      try {
        setLoading(true);
        const response = await axios.get('/api/education/marks-entries');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      if (isEditing) {
        await axios.put(`/api/education/marks-entries/${currentId}`, formData);
        toast.success('Marks entry updated successfully');
      } else {
        await axios.post('/api/education/marks-entries', formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this marks entry?')) {
      try {
        await axios.delete(`/api/education/marks-entries/${id}`);
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      setLoading(true);
      await axios.post('/api/education/marks-entries/upload', formData, {
        headers: {
  ...
  const handleExcelDownload = async () => {
    try {
      const response = await axios.get('/api/education/marks-entries/download', {
        responseType: 'blob'
      });
