...
const SecondAssessment = () => {
  ...
  const fetchSecondAssessments = async () => {
    try {
      let response;
      if (filter === 'all') {
        response = await axios.get('/api/education/reassessments');
      } else {
        response = await axios.get(`/api/education/reassessments?status=${filter}`);
      }
  ...
  const handleUpdateMarks = async (reassessmentId) => {
    ...
    try {
      await axios.put(`/api/education/reassessments/${reassessmentId}/update-marks`, {
