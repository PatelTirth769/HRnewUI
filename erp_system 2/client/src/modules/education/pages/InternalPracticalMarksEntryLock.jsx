...
function InternalPracticalMarksEntryLock() {
  ...
  useEffect(() => {
    const fetchMarksLockEntries = async () => {
      try {
        setLoading(true);
        const response = await axios.get('/api/education/marks-lock-entries');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      if (isEditing) {
        await axios.put(`/api/education/marks-lock-entries/${currentId}`, formData);
        toast.success('Marks lock entry updated successfully');
      } else {
        await axios.post('/api/education/marks-lock-entries', formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this marks lock entry?')) {
      try {
        await axios.delete(`/api/education/marks-lock-entries/${id}`);
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      setLoading(true);
      await axios.post('/api/education/marks-lock-entries/upload', uploadFormData, {
        headers: {
  ...
  const handleExcelDownload = async () => {
    try {
      const response = await axios.get('/api/education/marks-lock-entries/download', {
        responseType: 'blob'
      });
