...
const SetGracingCondonationRule = () => {
  ...
  useEffect(() => {
    const fetchRules = async () => {
      try {
        const response = await axios.get('/api/education/gracing-condonation-rule');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('/api/education/gracing-condonation-rule', { ...formData, rules });
