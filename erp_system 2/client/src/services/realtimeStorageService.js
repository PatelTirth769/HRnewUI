/**
 * Realtime Storage Service for handling localStorage operations with real-time updates
...
/**
 * Set data in localStorage and broadcast the change
...
/**
 * Update specific item in an array stored in localStorage and broadcast the change
...
export const updateStorageItem = (key, itemId, newData, idField = 'id') => {
  try {
    ...
    if (Array.isArray(data)) {
      ...
      // Broadcast the change
      createStorageEvent(key, updatedData);
      return true;
...
/**
 * Delete an item from an array stored in localStorage and broadcast the change
 * @param {string} key - The localStorage key
...
export const deleteStorageItem = (key, itemId, idField = 'id') => {
  try {
    ...
    if (Array.isArray(data)) {
      ...
      // Broadcast the change
      createStorageEvent(key, filteredData);
      return true;
    } else if (typeof data === 'object' && data !== null) {
      // If data is an object with nested arrays (like tabContent)
      const updatedData = { ...data };
...
/**
 * Add an item to an array stored in localStorage and broadcast the change
 * @param {string} key - The localStorage key
...
/**
 * Clear all data for a specific key in localStorage and broadcast the change
 * @param {string} key - The key to clear
 * @param {any} defaultValue - Default value to set after clearing (default: null)
 * @returns {boolean} - True if successful, false otherwise
 */
export const clearStorageData = (key, defaultValue = null) => {
  try {
    localStorage.removeItem(key);
    // Broadcast the change
    createStorageEvent(key, defaultValue);
    return true;
  } catch (error) {
...
/**
 * Register a listener for changes to a specific localStorage key
 * @param {string} key - The localStorage key to listen for changes
 * @param {Function} callback - The callback function to call when the key changes
 * @returns {Function} - A function to unregister the listener
 */
export const registerStorageListener = (key, callback) => {
